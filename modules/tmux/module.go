...
func (m *Module) Install(ctx *modules.InstallContext) error {
	...
	if err := os.WriteFile(hooksPath, []byte(tmuxHooksConfig), 0644); err != nil {
		return fmt.Errorf("write tmux hooks config: %w", err)
	}
...
func addToTmuxConf(ctx *modules.InstallContext, tmuxConfPath, sourceLine string) error {
	var content []byte
...
func removeFromTmuxConf(ctx *modules.InstallContext, tmuxConfPath, hooksPath string) {
	if _, err := os.Stat(tmuxConfPath); err != nil {
		return
	}
