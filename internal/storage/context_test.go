...
func TestListEventsContext(t *testing.T) {
	...
	ctx := context.Background()
	events, err := storage.QueryEventsContext(ctx, QueryOptions{})
	if err != nil {
		t.Fatalf("QueryEventsContext() error: %v", err)
	}
...
func TestListEventsContextWithFilters(t *testing.T) {
	...
	gitEvents, err := storage.QueryEventsContext(ctx, QueryOptions{Source: string(events.SourceGit)})
	if err != nil {
		t.Fatalf("QueryEventsContext() error: %v", err)
	}
	if len(gitEvents) != 5 {
	...
	limitedEvents, err := storage.QueryEventsContext(ctx, QueryOptions{Limit: 2})
	if err != nil {
		t.Fatalf("QueryEventsContext() error: %v", err)
	}
